...
function Template() {
    ...
    function loadTemplate(metadata, callback, context) {
        ...
        _.chain(metadata.fields)
        .filter(function(item) {
            return item.context == context || !item.context;
        })
        .each(function (field, index) {
            ...
            getTemplateInfo(fieldInfo, context || 'field', writable, function() {
    ...
    function getTemplateInfo(fieldInfo, context, writable, callback) {
        typeLoadEmitter.once(fieldInfo.field.type, function() {
            if (typeLookup[fieldInfo.field.type][context]
                ...
                var handlebarsInfo = {
                    ...
                    typeName: fieldInfo.field.type
                };
                if (fieldInfo.field.type == '/subType') {
                    handlebarsInfo.subTypes = fieldInfo.metadata.subTypes;
                }

                fieldInfo.template = Handlebars.compile(html)(handlebarsInfo);
                callback();
            } else {
        ...
        if (!typeLookup[fieldInfo.field.type]) {
            ...
            typeResourceLoader.load(fieldInfo.field.type, function(err, content) {
                if (err) throw err;
